...
export const PricingGrid: React.FC<PricingGridProps> = ({ state, forceDesktop = false }) => {
  ...
  const layoutClass = state.design.layout === 'grid' 
    ? gridClasses 
    : (forceDesktop ? forcedFlexClasses : flexClasses);

  return (
    <div 
      className={`flex gap-6 items-stretch ${layoutClass}`}
    >
